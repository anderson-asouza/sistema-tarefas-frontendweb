...
import { SvgPrinter, SvgEdit, SvgTrash, SvgCancel } from "../../components/outros/Svg";
import { useLocation, useNavigate  } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
import Loading from '../../components/outros/Loading';
import { ValidaRetornoAPI, TraduzMensagemErroBackend, GetDadosLogin, ChamarSplash, tMobile, ITENS_POR_PAGINA, MAX_CAMPO, PRN_MARGEM, DEF_MASSA_MODELOSTAREFA } from '../../util/servico';
import clsx from 'clsx';
import logo from '../../img/logo.png';

function LstModelosTarefa() {
  ...
  const [filtroNomeModeloTarefa, setFiltroNomeModeloTarefa] = useState('');
  const [filtroEffectNomeModeloTarefa, setFiltroEffectNomeModeloTarefa] = useState('');
  
  const [nomeModeloTarefa, setNomeModeloTarefa] = useState('');

  const itensFiltrados = useMemo(() => {
    return dados?.filter((c) =>
      c.mtarNome.toLowerCase().includes(filtroNomeModeloTarefa.toLowerCase())
    ) || [];
  }, [dados, filtroNomeModeloTarefa]);
  ...
  useEffect(() => {
    ...
    const buscarDados = async () => {
      ...
      try {
        ...
        const url = new URL(`${URL_API}ModelosTarefa`);
        if (countMassaDados.current > DEF_MASSA_MODELOSTAREFA)
          url.searchParams.append("nomeModeloTarefa", filtroEffectNomeModeloTarefa || "");
    ...
    window.addEventListener("atualizarListaModelosTarefa", buscarDados);

    return () => {  window.removeEventListener("atualizarListaModelosTarefa", buscarDados);
  };
  }, [t, Alerta, filtroEffectNomeModeloTarefa]);
  ...
  function handImprimir() {
    ...
    const linhaFiltro = (filtroNomeModeloTarefa) ? `${t('lstModelosTarefa.filtro')}: ${filtroNomeModeloTarefa}` : "";    
  ...
  const gerarPdf = async (linhaFiltro) => {
    ...
    body.push([
      { text: t('lstModelosTarefa.nome'), style: 'tableHeader' },
      { text: t('lstModelosTarefa.descricao'), style: 'tableHeader' },
    ]);
    ...
    const docDefinition = {
      ...
      header: {
        columns: [
          ...
          {
            stack: [
              { text: t('lstModelosTarefa.titulo'), style: 'header', alignment: 'left', margin: [PRN_MARGEM+5, PRN_MARGEM, PRN_MARGEM, 0] },
  ...
  async function handleExcluir() {
    const confirmado = await Pergunta({ mensagem: t('lstModelosTarefa.desejaExcluir') });
    ...
    try {
      ...
      const url = new URL(`${URL_API}ModelosTarefa/${itemSelecionado?.[primaryKey]}`);
      ...
      ChamarSplash(t('lstModelosTarefa.excluido') + ' ' + itemSelecionado.mtarNome, "aviso");
      handleDesmarcar();
    } catch (err) {
      Alerta({ mensagem: err });
    }
  ...
  function handleEditar() {
    navigate("/cadastromodelotarefa", { state: { backgroundLocation: location, itemSelecionado }});
    handleDesmarcar();
  }

  function Filtrar()
  {
    setFiltroNomeModeloTarefa(nomeModeloTarefa);

    if (countMassaDados.current > DEF_MASSA_MODELOSTAREFA)
      setFiltroEffectNomeModeloTarefa(nomeModeloTarefa);
  }
  ...
  return (
    <>
      ...
      <div className={stylesLst.container}>
        <h1> <TbTemplate style={{ transform: 'scale(1.4)', transformOrigin: 'center', marginRight: '0.4em' }} /> {t('lstModelosTarefa.titulo')}</h1>
        <div className="colunas quebraLinhaMobile">        
          <div className="colunaEsquerda">
            <label className={stylesLst.labels} htmlFor="nomeModeloTarefa">{t('lstModelosTarefa.filtro')}:</label>
            <input type="text" id="nomeModeloTarefa" name="nomeModeloTarefa" placeholder={t('lstModelosTarefa.filtroModeloTarefaHolder')} maxLength={MAX_CAMPO} value={nomeModeloTarefa} onChange={(e) => setNomeModeloTarefa(e.target.value)} className={stylesLst.filtro} />
        ...
        (
          <div>
            <table className={stylesLst.listaItens}>
              <thead>
                <tr>
                  <th style={!isMobile ? { width: '40em' } : { width: '10em' }}> {tMobile(t, "lstModelosTarefa.nome")}</th>
                  <th style={!isMobile ? { width: 'auto' } : { width: '10em' }}> {tMobile(t, "lstModelosTarefa.descricao")}</th>
                </tr>
              </thead>
              <tbody>
...
export default LstModelosTarefa;
